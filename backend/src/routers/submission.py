...
@router.get("/exam-withscore/{exam_id}/students")
def get_exam_submissionsscore_with_students(exam_id: int):
    """
    Get all submissions for an exam, including students who are enrolled but haven't submitted.
    Returns score, score_grade, and overall_feedback from the submission table.
    """
    try:
        with get_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                # First, get the course_id and exam details for this exam
                cur.execute(
                    """
                    SELECT course, date, end_time
                    FROM exams
                    WHERE id = %s
                    """,
                    (exam_id,),
                ...
                # Get all submissions for this exam with scores
                cur.execute(
                    """
                    SELECT
                        s.id as submission_id,
                ...
                # Add all submissions with their scores
                for sub in submissions:
                    result.append(
                        {
                            "submission_id": sub["submission_id"],
                            ...
                            "student_email": sub["student_email"],
                            "status": sub["status"],
                            "submission_date": str(sub["submission_date"]) if sub["submission_date"] else None,
                            "submission_time": str(sub["submission_time"]) if sub["submission_time"] else None,
                            "score": sub["score"],
...
def get_exam_submissions(exam_id: int):
    ...
    try:
        ...
        for sub in submissions:
            if sub.get("submission_time") and not isinstance(
                sub["submission_time"], str
            ):
                sub["submission_time"] = sub["submission_time"].strftime("%H:%M:%S")
            if sub.get("submission_date") and not isinstance(
                sub["submission_date"], str
            ):
...
def get_submission(submission_id: int):
    ...
    try:
        ...
        # Convert time objects to strings
        if submission.get("submission_time") and not isinstance(
            submission["submission_time"], str
        ):
            submission["submission_time"] = submission["submission_time"].strftime(
                "%H:%M:%S"
            )
        if submission.get("submission_date") and not isinstance(
            submission["submission_date"], str
        ):
