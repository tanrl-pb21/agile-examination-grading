...
class ExamService:
    ...
    def add_exam(
        self, title, exam_code, date, start_time, end_time, status="scheduled"
    ):
        if not title:
        ...
        with get_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    sql,
                    (title, exam_code, date, start_time, end_time, duration, status),
                )
                row = cur.fetchone()
    ...
    def update_exam(
        self, exam_id, title, exam_code, date, start_time, end_time, status="scheduled"
    ):
        if not title:
            raise ValueError("Title is required")
        if not exam_code:
